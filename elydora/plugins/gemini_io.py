...
from .gemini_command import same_gemini_agent_id, same_gemini_path
...
def read_gemini_document() -> GeminiDocument:
    ...
    physical_directory_exists(
        os.path.dirname(file_path), "Gemini CLI configuration directory"
    )
    snapshot = read_physical_file(
        file_path, "Gemini CLI user settings", MAX_SOURCE_BYTES
    )
    if snapshot is None:
...
def _validate_runtime_config(
    ...
    if (
        not same_gemini_agent_id(agent_id, expected_agent_id)
        or config.get("agent_name") != AGENT_KEY
    ):
        raise ValueError(
            "Elydora runtime identity does not match Gemini CLI hooks: "
            f"{config_path}"
        )
    if "token" in config:
...
def validate_runtime_tree(agent_directory: str, agent_id: str) -> None:
    ...
    if (
        ...
        or not same_gemini_agent_id(configured_id, agent_id)
        or config.get("agent_name") != AGENT_KEY
    ):
        raise ValueError(
            "Elydora runtime config identity does not match Gemini CLI agent "
            f"{agent_id}: {config_path}"
        )


def _runtime_contract_exists(contract: GeminiRuntimeContract) -> bool:
    root = elydora_dir()
    agent_directory = os.path.dirname(contract.guard_path)
    if (
        not same_gemini_path(os.path.dirname(agent_directory), root)
        or not same_gemini_path(
...
def gemini_runtime_files_exist(
    contracts: List[GeminiRuntimeContract],
) -> bool:
