...
from .gemini_command import same_gemini_path
...
def _agent_paths(config: InstallConfig) -> GeminiRuntimePaths:
    ...
    if not same_gemini_path(config.get("guard_script_path", ""), guard_path):
        raise ValueError(
            "Elydora guard runtime must use the managed agent directory: "
            f"{guard_path}"
        )
    return GeminiRuntimePaths(agent_id, agent_directory, guard_path, audit_path)
...
def _validate_install_config(config: InstallConfig) -> None:
    ...
    if config.get("agent_name") != AGENT_KEY:
        raise ValueError(
            f"Gemini CLI installation requires agent_name {AGENT_KEY}"
        )
