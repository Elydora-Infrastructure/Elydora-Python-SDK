...
from .letta_command import same_letta_agent_id, same_letta_path
from .letta_contract import AGENT_KEY, LettaRuntimeContract, elydora_dir
...
def _validate_runtime_config(
    ...
    if (
        not same_letta_agent_id(agent_id, expected_agent_id)
        or config.get("agent_name") != AGENT_KEY
    ):
        raise ValueError(
            "Elydora runtime identity does not match Letta Code hooks: "
            f"{config_path}"
        )
    if "token" in config:
...
def validate_runtime_tree(agent_directory: str, agent_id: str) -> None:
    ...
    if (
        ...
        or not same_letta_agent_id(configured_id, agent_id)
        or config.get("agent_name") != AGENT_KEY
    ):
        raise ValueError(
            "Elydora runtime config identity does not match Letta Code agent "
            f"{agent_id}: {config_path}"
        )


def _runtime_contract_exists(contract: LettaRuntimeContract) -> bool:
    root = elydora_dir()
    agent_directory = os.path.dirname(contract.guard_path)
    if (
        not same_letta_path(os.path.dirname(agent_directory), root)
        or not same_letta_path(
