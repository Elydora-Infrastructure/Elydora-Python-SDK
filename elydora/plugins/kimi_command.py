"""Kimi hook command rendering and exact runtime ownership."""
...
def same_kimi_path(left: str, right: str) -> bool:
    return os.path.normcase(os.path.abspath(left)) == os.path.normcase(
        os.path.abspath(right)
    )


def same_kimi_agent_id(left: str, right: str) -> bool:
...
def build_kimi_command(script_path: str) -> str:
    if not os.path.isabs(sys.executable) or not os.path.isabs(script_path):
        raise ValueError(
            "Kimi hook commands require absolute executable and script paths"
        )
    if os.name == "nt":
...
def kimi_runtime_reference(
    ...
    if not same_kimi_path(os.path.dirname(agent_directory), runtime_root()):
        return None
    agent_id = os.path.basename(agent_directory)
    if agent_id in {"", ".", ".."}:
        return None
    return KimiRuntimeReference(agent_id, script_path)
