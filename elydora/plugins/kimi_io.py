...
def resolve_kimi_contracts() -> List[KimiContract]:
    ...
    if same_kimi_path(stable.config_path, legacy.config_path):
        return [stable]
...
def _validate_runtime_config(
    ...
    if (
        not same_kimi_agent_id(agent_id, expected_agent_id)
        or config.get("agent_name") != AGENT_KEY
    ):
        raise ValueError(
            f"Elydora runtime identity does not match Kimi hooks: {config_path}"
        )
    if "token" in config:
...
def validate_runtime_tree(agent_directory: str, agent_id: str) -> None:
    ...
    if (
        ...
        or not same_kimi_agent_id(configured_id, agent_id)
        or config.get("agent_name") != AGENT_KEY
    ):
        raise ValueError(
            f"Elydora runtime config identity does not match Kimi agent {agent_id}: "
            f"{config_path}"
        )


def _runtime_contract_exists(contract: KimiRuntimeContract) -> bool:
    root = runtime_root()
    agent_directory = os.path.dirname(contract.guard_path)
    if (
        not same_kimi_path(os.path.dirname(agent_directory), root)
        or not same_kimi_path(
