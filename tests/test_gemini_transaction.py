...
from elydora.plugins import _transaction, gemini_installation
from elydora.plugins.gemini_config import render_gemini_document
from elydora.plugins.gemini_contract import (
    AUDIT_HOOK_NAME,
    GUARD_HOOK_NAME,
    build_gemini_group,
)
from elydora.plugins.gemini_io import read_gemini_document
...
def prepare_installation(fixture: object) -> list[_transaction.FileChange]:
    ...
    paths = gemini_installation.preflight_gemini_installation(
    ...
    return gemini_installation.prepare_gemini_installation(
...
def test_transaction_rolls_back_all_five_files(
    ...
    def fail_settings_once(source: str, destination: str) -> None:
        ...
        if os.path.abspath(destination) == str(fixture.config_path) and not failed:
            failed = True
            raise OSError("injected Gemini settings failure")
        real_replace(source, destination)

    monkeypatch.setattr(_transaction.os, "replace", fail_settings_once)

    with pytest.raises(OSError, match="injected Gemini settings failure"):
        fixture.install()
...
def test_concurrent_settings_change_is_detected_before_runtime_commit(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fixture = prepare_fixture(
        monkeypatch, tmp_path, existing_settings={"theme": "GitHub"}
    )
    changes = prepare_installation(fixture)
    concurrent = '{"theme":"Atom One","hooks":{"Notification":[]}}\n'
    write_text(fixture.config_path, concurrent)

    with pytest.raises(OSError, match="changed during installation"):
        gemini_installation.commit_gemini_installation(changes)

    assert fixture.source() == concurrent
    for name in RUNTIME_NAMES:
        assert not (fixture.agent_dir / name).exists()
    assert_no_transaction_files(fixture.home_dir)
...
def test_mismatched_runtime_identity_fails_before_writes(
    ...
    write_json(fixture.runtime_config_path, {
        "agent_id": "another-agent", "agent_name": "gemini"
    })
...
@pytest.mark.parametrize(
    ("field", "value", "pattern"),
    [
        ("agent_name", "codex", "requires agent_name gemini"),
...
def test_cli_preflight_preserves_malformed_settings(tmp_path: Path) -> None:
    ...
    environment = {
        **os.environ,
        ...
        "GEMINI_CLI_HOME": str(home_dir),
    }

    result = subprocess.run(  # nosec B603
        [
            sys.executable, "-m", "elydora.cli", "install",
            "--agent", "gemini", "--org_id", "org-1",
    ...
    assert "parse Gemini CLI user settings" in result.stderr
