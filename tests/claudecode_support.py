...
from elydora.plugins.claudecode import ClaudeCodePlugin
...
def prepare_fixture(
    ...
    *,
    ...
    home_dir = tmp_path / "home with spaces and 'quote %CLAUDE_HOOK_EVENT%"
    project_dir = tmp_path / "project with spaces"
    project_dir.mkdir(parents=True)
    monkeypatch.setenv("HOME", str(home_dir))
    monkeypatch.setenv("USERPROFILE", str(home_dir))
    monkeypatch.chdir(project_dir)
    if config_override is MISSING:
        monkeypatch.delenv("CLAUDE_CONFIG_DIR", raising=False)
        config_directory = home_dir / ".claude"
    else:
        configured = (
            str(home_dir / "custom claude")
            if config_override is None
            else str(config_override)
        )
        monkeypatch.setenv("CLAUDE_CONFIG_DIR", configured)
        config_directory = Path(os.path.abspath(configured))
...
def assert_native_handler(
    handler: Mapping[str, Any], script_path: Path, status_message: str
) -> None:
    assert set(handler) == {
        "type", "command", "args", "timeout", "statusMessage",
    }
    ...
    assert handler["args"] == [str(script_path)]
    assert handler["timeout"] == 10
    assert handler["statusMessage"] == status_message


def run_handler(
    handler: Mapping[str, Any], payload: bytes, fixture: ClaudeFixture
) -> subprocess.CompletedProcess[bytes]:
    return subprocess.run(
        [str(handler["command"]), *map(str, handler["args"])],
        input=payload,
        capture_output=True,
        check=False,
